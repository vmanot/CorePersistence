...
extension XMLKeyedDecodingContainer {
    ...
    private func decodeConcrete<T: Decodable>(
        ...
        default:
            switch type {
            case is _XMLAttributePropertyProtocol.Type:
                box = try getAttributeBox(for: type, attributes, key)
        ...
        } else if box.isNull, let type = type as? _OptionalXMLAttributePropertyProtocol.Type, let nullAttribute = type.init() as? T {
            value = nullAttribute
        } else {
    ...
    private func getAttributeBox<T: Decodable>(for type: T.Type, _ attributes: [KeyedBox.Attribute], _ key: Key) throws -> Box {
        ...
        if type is AnyOptional.Type || type is _OptionalXMLAttributePropertyProtocol.Type { return NullBox() }
